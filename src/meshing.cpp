...
namespace HexMesher
{
  ...
  private:
    ...
    {
      std::vector<Slice>& slices = slices_per_axis[int(axis)];
    ...
    std::pair<double, double> slice_extrema(Axis axis)
    {
      ...
      for(std::size_t slice(0); slice < slices_per_axis[int(axis)].size() - 1; slice++)
      {
        min = std::min(min, slices_per_axis[int(axis)][slice + 1].coord - slices_per_axis[int(axis)][slice].coord);
        max = std::max(max, slices_per_axis[int(axis)][slice + 1].coord - slices_per_axis[int(axis)][slice].coord);
      }
    ...
    void fix_aspect_ratios(Axis axis)
    {
      ...
      for(std::size_t slice(0); slice < slices.size() - 1; slice++)
      {
        ...
        const std::size_t new_segments = aspect_ratio / 1.5;
        const double new_width = width / new_segments;

        if(new_segments > 0)
        {
          for(std::size_t i(0); i < new_segments - 1; i++)
          {
            new_slices.push_back(slices[slice].coord + (i + 1) * new_width);
          }
        }
      }
    ...
    void choose_subdivision_levels(Mesh& mesh, std::uint64_t levels)
    {
      ...
      for(std::size_t slice(0); slice < unbuffered_slices.size() - 1; slice++)
      {
        ...
        */
        ...
        const std::uint64_t available_transition_slices = std::uint64_t(std::floor(available_width / buffer_width));
        ...
        {
          add_slice(Axis::Z, Slice(start + dir * (i + 1) * buffer_width, start_level - i - 1));
        }
      }
  ...
  VolumeMesh fbm_mesh(Mesh& mesh, AABBTree& aabb_tree, std::uint64_t levels)
  {
    ...
    const std::size_t num_buckets_x = std::size_t(std::ceil((bb.max.x - bb.min.x) / mg.gap));
    const std::size_t num_buckets_y = std::size_t(std::ceil((bb.max.y - bb.min.y) / mg.gap));
    const std::size_t num_buckets_z = std::size_t(std::ceil((bb.max.z - bb.min.z) / mg.gap));
    ...
    // We also know how many regular refinemets are intended.
    ...
    const auto make_intervals = [&](const std::vector<std::optional<double>>& buckets)
    {
      ...
      BucketIterator current = buckets.begin();
    ...
    const auto merge_empty = [&](std::vector<Interval>& intervals)
    {
      ...
      while(iter != intervals.end())
      {
        ...
        else
        {
          ...
          Iter prev = std::prev(iter);
          Iter next = std::next(iter);
    ...
    const auto merge_too_small = [&](std::vector<Interval>& intervals)
    {
      ...
      while(iter != intervals.end())
      {
        ...
        if(size * mg.gap < 0.01 * mesh_size || size < 3)
        {
    ...
    {
      builder.add_slice(Axis::X, bb.min.x + mg.gap * std::distance(buckets_x.cbegin(), interval.begin));
    }
    builder.add_slice(Axis::X, bb.max.x);

    for(const Interval& interval : intervals_y)
    {
      builder.add_slice(Axis::Y, bb.min.y + mg.gap * std::distance(buckets_y.cbegin(), interval.begin));
    }
    builder.add_slice(Axis::Y, bb.max.y);

    for(const Interval& interval : intervals_z)
    {
      builder.add_slice(Axis::Z, bb.min.z + mg.gap * std::distance(buckets_z.cbegin(), interval.begin));
    }
